...
Options:
  --nx-target=N       Base Gmsh NxTarget. Four levels use
  ...
  --geo=PATH          Parameterized periodic Gmsh geometry. Default:
                      examples/meshes/periodic_box_unstructured.geo
  --mesh-dir=PATH     Generated VTK mesh directory. Default:
...
Method:
  - axis-aligned periodic box defined by the Gmsh geometry
  - periodic boundary pairs 1<->2, 3<->4, and 5<->6
  - x-directed analytical plane wave
  - unstructured Gmsh family with target h, h/2, h/4, and h/8
...
Interpretation:
  On a general unstructured tetrahedral family, nominally zero transverse
  components can be generated at order N and reduce the aggregate electric
  rate. Inspect the Ez and Hy component rates and the transverse leakage
  printed below the aggregate table.
""")
end
...
function run_distributed_periodic_convergence_case(
    ...
    MPI.Barrier(comm)
    local_elapsed = @elapsed advance_distributed_periodic_convergence_case!(
        U,
        distributed_dg,
        periodic,
        registry,
    ...
    h_min, h_max = periodic_distributed_h_min_h_max(distributed_dg)
    ...
    result = DistributedPeriodicConvergenceResult(
        ...
        min_owned,
        max_owned,
        # periodic_distributed_characteristic_h(distributed_dg),
        characteristic_h,
        h_min,
        h_max,
        dt,
        ...
        for result in subset
            ...
            if previous !== nothing
                ...
                component_rates = ntuple(
                    component -> periodic_convergence_rate(
...
function print_periodic_convergence_results(
    ...
    println()
    println("Plane-wave component and polarization-leakage diagnostics")
    println("---------------------------------------------------------")
    println(
        rpad("N", 4),
        rpad("level", 7),
    ...
    for result in results
        ...
        println(
            ...
            rpad(formatted_periodic_rate(result.rate_ez), 10),
            rpad(@sprintf("%.3e", result.l2_hy_error), 13),
            rpad(formatted_periodic_rate(result.rate_hy), 10),
...
function periodic_convergence_csv_header()
    return (
        "mpi_ranks,boundary_condition,order,esprk_order,cubature_order,mesh_level," *
        ...
        "characteristic_h,h_min,h_max,dt,nsteps,elapsed_seconds,l2_electric_error," *
        "l2_magnetic_error,l2_total_error,l2_ex_error,l2_ey_error," *
        "l2_ez_error,l2_hx_error,l2_hy_error,l2_hz_error," *
...
function periodic_convergence_verdict(
    ...
    for order in sort(unique(result.order for result in results))
        ...
        minimum_electric =
            expected_electric - PERIODIC_CONVERGENCE_RATE_TOLERANCE
        minimum_magnetic =
            expected_magnetic - PERIODIC_CONVERGENCE_RATE_TOLERANCE
        electric_pass =
        ...
        verdict &= electric_pass && magnetic_pass
        push!(
            messages,
            @sprintf(
                "N=%d: aggregate E rate=%s, strict target %.1f (minimum %.1f): %s; aggregate H rate=%s, target %.1f (minimum %.1f): %s",
                order,
                formatted_periodic_rate(final_rate_electric),
                expected_electric,
                minimum_electric,
                electric_pass ? "PASS" : "FAIL",
                formatted_periodic_rate(final_rate_magnetic),
                expected_magnetic,
                minimum_magnetic,
...
function run_periodic_convergence_study(
    ...
    for order in config.orders
        for (level_index, nx_target) in enumerate(config.nx_targets)
            ...
            if rank == 0
                @printf(
                    "Running N=%d, ESPRK=%d, level=%d, NxTarget=%d\n",
                    order,
                    order + 1,
                    mesh_level,
                    nx_target,
