...
introduces no auxiliary unknowns. In its current dimensionless homogeneous
vacuum form,
\begin{align}
...
\begin{align}
  \vect P_H
    ...
    {a|\vect H|^2+(1-a)|\vect E|^2+\delta},\\
  \vect P_E
    &=
    -\frac{\vect E\times\vect H}
    {a|\vect E|^2+(1-a)|\vect H|^2+\delta}.
\end{align}
...
with
\[
  D_H=a|\vect H|^2+(1-a)|\vect E|^2+\delta,\qquad
  D_E=a|\vect E|^2+(1-a)|\vect H|^2+\delta.
\]
...
distributed_maxwell_nonlinear_pml_rk_step!(
    ...
    HesthavenWarburtonFormulation(:upwind), pml,
)
\end{lstlisting}

The periodic wrapper is
\code{distributed\_periodic\_maxwell\_nonlinear\_pml\_rk\_step!}. On MPI
ranks, PML sources are added only to owned elements and ghost residuals remain
zero. Because no additional state variables or traces are introduced, the
existing six-component halo exchange is sufficient.

The nonlinear PML is dissipative and is therefore advanced with the explicit
RK family. For \code{PoissonBracketFormulation}, use
\code{run\_maxwell\_nonlinear\_pml\_time\_steps!} or its distributed
counterpart rather than the conservative ESPRK driver. The current PML
wrapper fixes
$\varepsilon=\mu=1$; coupling this formulation consistently to heterogeneous
materials is future work.
...
shortest global domain extent. PML runs require
\code{--epsilon=1 --mu=1} and switch from ESPRK to ordinary explicit RK.
...
\begin{itemize}[leftmargin=*]
  \item \code{energy.csv}: DG mass-matrix electric, magnetic, and total
        energy;
  \item \code{quadrature\_diagnostics.csv}: independent-cubature field and
        energy errors, optical chirality, electric and magnetic charge, and
        linear and angular momentum;
  \item \code{fields.pvd} and parallel VTU pieces: ParaView time series;
  \item \code{integration\_points\_rankNNNN.csv}: final integration-point
        numerical and analytical values, including pointwise chirality;
  \item \code{run\_metadata.toml} and \code{partition\_metadata.csv};
...
\begin{itemize}[leftmargin=*]
  \item \code{energy.csv}: DG mass-matrix electric, magnetic, and total energy;
  \item \code{quadrature\_diagnostics.csv}: independent-cubature field and
        energy errors, optical chirality, electric and magnetic charge, and
        linear and angular momentum;
  \item \code{fields.pvd} and parallel VTU pieces: ParaView time series;
...
\code{round(cells * ranks^(1/3))} cells per axis so the element count grows
...
\begin{itemize}[leftmargin=*]
  \item \code{run\_metadata.toml}: configuration, Julia version, MPI size,
        time-step data, elapsed solve time, and final checkpoint;
  \item \code{partition\_metadata.csv}: owned and ghost elements, local nodes,
        neighbor ranks, interface-face counts, halo-size ratios, send/receive
        buffer sizes, and global element ranges;
...
\begin{itemize}[leftmargin=*]
  ...
  \item The nonlinear PML currently supports only the dimensionless
        homogeneous-vacuum system with $\varepsilon=\mu=1$; heterogeneous
        material coupling and anisotropic damping tensors are not implemented.
  \item The nonlinear PML path supports explicit RK integration, not ESPRK,
        because damping breaks the conservative Poisson-bracket structure.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Extend the nonlinear PML to heterogeneous materials and validate
        reflection errors for oblique incidence and corner layers.
...
mpiexec -n 2 julia --project=. \
  examples/distributed_poisson_bracket_maxwell.jl \
  --boundary-condition=pmc \
  --pml-width=0.2 --pml-sigma-max=12 --rk-order=4
